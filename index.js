...
function findElementGreaterThan(arr, num) {
  ...
  const findElement = arr.find((elem) => elem > num);
...
function findLastEvenNumber(arr) {
  ...
  const findLastIndex = arr.findLast((num) => num % 2 === 0);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const sliceIndex = arr.slice(startIdx);
...
function getPositiveArrayValues(arr) {
  ...
  const PositiveNumFilter = valuesArray.filter(
...
function removeAndDouble(arr, startIdx) {
  ...
  // Подвоюємо значення кожного видаленого елемента
  const doubledElements = arr.map((element) => element * 2);

  // Повертаємо масив подвоєних значень видалених елементів
...
function sumAndPrint(arr) {
  ...
  const result = arr.reduce((num, elem) => {
    return num + elem;
  });
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flattenedArray = arr.flat().map((num) => num ** 2);

  const filteredArray = flattenedArray.filter((num) => num > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const slicedElem = arr.slice(-n);

  const reduceElem = slicedElem.reduce((acc, num) => acc + num);

  slicedElem.forEach((elem) => console.log(elem));
...
function findDuplicateElements(arr) {
  ...
  const filterDouble = arr.filter((elem, index) => arr.indexOf(elem) !== index);
...
function capitalizeWords(arr) {
  ...
  const bigFirstLetter = arr.map((elem, index, array) => {
    const firstLetter = elem.charAt(0).toUpperCase();

    const restOfWord = elem.slice(1);
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((acc, elem) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  const findLastNegativeNum = arr.findLastIndex((elem) => elem < 0);

  return findLastNegativeNum;
}
